...
@Component
public class SertImageGoodsBatchRepositoryImpl {

    @Autowired
    private JdbcTemplate jdbcTemplate;

    @Transactional
    public void batchUpsertSertImageGoods(List<SertImageGoods> rows) {
        if (rows == null || rows.isEmpty()) return;

        String sql = """
            INSERT INTO sert_image_goods (uid_sert_image, uid_good, is_del)
            VALUES (?, ?, ?)
            ON CONFLICT (uid_sert_image, uid_good)
            DO UPDATE SET
        ...
        jdbcTemplate.batchUpdate(sql, rows, batchSize, (ps, r) -> {
            ...
            ps.setString(2, r.getGood().getUid());
